@mod @mod_idea
Feature: Users can be required to specify certain fields when adding entries to ideabase activities
  In order to constrain user input
  As a teacher
  I need to specify certain fields as required when I add entries to ideabases
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | idea     | Test ideabase name | n     | C1     | idea1    |
    And I log in as "teacher1"
    And I follow "Course 1"
    And I add a "Text input" field to "Test ideabase name" ideabase and I fill the form with:
      | Field name | Base Text input |
      | Required | yes |
      | Field description | Base Text input |
    And I add a "Checkbox" field to "Test ideabase name" ideabase and I fill the form with:
    ...
    """
    And I press "Add"
    And I add a "Latlong" field to "Test ideabase name" ideabase and I fill the form with:
      | Field name | Required Latlong |
      | Field description | Required Latlong |
      | Required | yes |
    And I add a "Menu" field to "Test ideabase name" ideabase and I fill the form with:
      | Field name | Required Menu |
      | Field description | Required Menu |
      | Required | yes |
      | Options | Option 1 |
    And I add a "Number" field to "Test ideabase name" ideabase and I fill the form with:
      | Field name | Required Number |
      | Field description | Required Number |
      | Required | yes |
    And I add a "Radio button" field to "Test ideabase name" ideabase and I fill the form with:
    ...
    And I add a "Text input" field to "Test ideabase name" ideabase and I fill the form with:
      ...
      | Required | yes |
    And I add a "Text area" field to "Test ideabase name" ideabase and I fill the form with:
      | Field name | Required Text area |
      | Field description | Required Text area |
      | Required | yes |
    And I add a "URL" field to "Test ideabase name" ideabase and I fill the form with:
      | Field name | Required URL |
      | Field description | Required URL |
      | Required | yes |
    And I add a "Multimenu" field to "Test ideabase name" ideabase and I fill the form with:
  ...
  Scenario: Students receive errors for empty required fields but not for optional fields
    ...
    And I follow "Course 1"
    And I add an entry to "Test ideabase name" ideabase with:
    ...
    And I should see "No entries in ideabase"
  ...
  Scenario: Students recieve no error for filled in required fields
    ...
    And I press "Save and view"
    And I follow "View list"
    Then I should not see "No entries in ideabase"
    And I should see "New entry text"

  Scenario: Fields refill with idea after having an error
    When I log in as "student1"
    And I follow "Course 1"
    And I add an entry to "Test ideabase name" ideabase with:
       | RTOC Option 1                 | 1                   |
  ...
  Scenario: A student fills in Latitude but not Longitude will see an error
    ...
    And I follow "Course 1"
    When I add an entry to "Test ideabase name" ideabase with:
       | Base Text input               | Some input to allow us to submit the otherwise empty form |
       | Required Checkbox Option 1    | 1                                                         |
       | RTOC Option 1                 | 1                                                         |
       | Latitude                      | 24                                                        |
