...
/**
 * idea generator class for mod_idea.
 *
 * @package    mod_idea
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_idea_generator extends testing_module_generator {

    /**
     * @var int keep track of how many ideabase fields have been created.
     */
    protected $ideabasefieldcount = 0;

    /**
     * @var int keep track of how many ideabase records have been created.
     */
    protected $ideabaserecordcount = 0;
    ...
    /**
     * To be called from idea reset code only,
     * do not use in tests.
     * @return void
     */
    public function reset() {
        $this->ideabasefieldcount = 0;
        $this->ideabaserecordcount = 0;

        parent::reset();
    }

    /**
     * Creates a mod_idea instance
     *
     * @param array $record
     * @param array $options
     * @return StdClass
    ...
    /**
     * Creates a field for a mod_idea instance.
     * Currently, the field types in the ignoredfieldtypes array aren't supported.
     *
     * @param StdClass $record
     * @param mod_idea $idea
     * @return idea_field_{type}
     */
    public function create_field(stdClass $record = null, $idea = null) {
        $record = (array) $record;
        ...
        $this->ideabasefieldcount++;

        if (!isset($idea->course)) {
            throw new coding_exception('course must be present in phpunit_util::create_field() $idea');
        }

        if (!isset($idea->id)) {
            throw new coding_exception('ideaid must be present in phpunit_util::create_field() $idea');
        } else {
        ...
        if (!isset($record['name'])) {
            $record['name'] = "testField - " . $this->ideabasefieldcount;
        }

        if (!isset($record['description'])) {
            $record['description'] = " This is testField - " . $this->ideabasefieldcount;
        }
        ...
        $field = idea_get_field($record, $idea);
        $field->insert_field();

        idea_generate_default_template($idea, 'addtemplate', 0, false, true);
    ...
    /**
     * Creates a field for a mod_idea instance.
     * Keep in mind the default idea field params created in create_field() function!
     * ...if you haven't provided your own custom idea field parameters there.
     * Currently, the field types in the ignoredfieldtypes array aren't supported.
     ...
     * @return idea_field_{type}
     */
    public function create_entry($idea, array $contents, $groupid = 0) {
        global $DB;

        $this->ideabaserecordcount++;

        $recordid = idea_add_record($idea, $groupid);
        ...
        foreach ($fields as $field) {
            ...
            $field = idea_get_field($field, $idea);
        ...
        foreach ($contents as $fieldid => $content) {
            $field = idea_get_field_from_id($fieldid, $idea);
