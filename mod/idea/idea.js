 ...
 * Used when editing a idea template
 */
...
function showHideAdvSearch(checked) {
    ...
    for(i=0;i<divs.length;i++) {
        if(divs[i].id.match('idea_adv_form')) {
            if(checked) {
...
M.idea_urlpicker.init = function(Y, options) {
    options.formcallback = M.idea_urlpicker.callback;
...
M.idea_urlpicker.callback = function (params) {
