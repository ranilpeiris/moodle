...
/**
 * Search area for mod_idea activity entries.
 *
 * @package    mod_idea
...
class entry extends \core_search\base_mod {
     ...
     */
    protected $entriesidea = array();

    /**
     * Returns recordset containing required idea for indexing ideabase entries.
     *
    ...
    public function get_document($entry, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('idea', $entry->ideaid, $entry->course);
            $context = \context_module::instance($cm->id);
        } catch (\dml_missing_record_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_idea ' . $entry->id . ' document, not all required idea is available: ' .
                $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        } catch (\dml_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_idea' . $entry->id . ' document: ' . $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        }

        // Prepare associative array with idea from DB.
        ...
        // All fields should be already returned as plain text by idea_field_base::get_content_value.
    ...
    public function check_access($id) {
        ...
        if (!has_capability('mod/idea:viewentry', $context)) {
            return \core_search\manager::ACCESS_DENIED;
        }

        $numberofentriesindb = $DB->count_records('idea_records', array('ideaid' => $entry->ideaid));
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/idea/view.php', array( 'd' => $entry->ideaid, 'rid' => $entry->id ));
    }

    /**
     * Link to the ideabase activity.
     *
     * @param \core_search\document $doc
     * @return \moodle_url
     */
    public function get_context_url(\core_search\document $doc) {
        $entry = $this->get_entry($doc->get('itemid'));
        return new \moodle_url('/mod/idea/view.php', array('d' => $entry->ideaid));
    }
    ...
    /**
     * Add the ideabase entries attachments.
     *
     * @param \core_search\document $doc
     * @return void
     */
    public function attach_files($doc) {
        ...
        $cm = $this->get_cm('idea', $entry->ideaid, $doc->get('courseid'));
        $context = \context_module::instance($cm->id);

        // Get the files and attach them.
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_idea', 'content', $entryid, 'filename', false);
    ...
    protected function get_entry($entryid) {
        ...
        if (empty($this->entriesidea[$entryid])) {
            $this->entriesidea[$entryid] = $DB->get_record('idea_records', array( 'id' => $entryid ), '*', MUST_EXIST);
        }

        return $this->entriesidea[$entryid];
    }
    ...
    protected function get_fields_for_entries($entry) {
        ...
        $sql = "SELECT dc.*, df.name AS fldname,
                  ...
                  FROM {idea_content} dc, {idea_fields} df
        ...
        $template = $DB->get_record_sql('SELECT addtemplate FROM {idea} WHERE id = ?', array($entry->ideaid));
        $template = $template->addtemplate;

        // Filtering out the idea_content records having invalid fieldtypes.
        ...
        // Filtering out contents which belong to fields that aren't present in the addtemplate of the ideabase activity instance.
        foreach ($filteredcontents as $content) {
    ...
    protected function get_field_class_name($fieldtype) {
        ...
        require_once($CFG->dirroot . '/mod/idea/field/' . $fieldtype . '/field.class.php');
        return 'idea_field_' . $fieldtype;
    }
}
