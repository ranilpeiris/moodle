...
/**
 * Define all the backup steps that will be used by the backup_idea_activity_task
 */

/**
 * Define the complete idea structure for backup, with file and id annotations
 */
class backup_idea_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        // Build the tree
        $idea->add_child($fields);
        $fields->add_child($field);

        $idea->add_child($records);
        ...
        // Define sources
        $idea->set_source_table('idea', array('id' => backup::VAR_ACTIVITYID));
        ...
        if ($userinfo) {
            $record->set_source_table('idea_records', array('ideaid' => backup::VAR_PARENTID));

            $content->set_source_table('idea_content', array('recordid' => backup::VAR_PARENTID));

            $rating->set_source_table('rating', array('contextid'  => backup::VAR_CONTEXTID,
                                                      'itemid'     => backup::VAR_PARENTID,
                                                      'component'  => backup_helper::is_sqlparam('mod_idea'),
                                                      'ratingarea' => backup_helper::is_sqlparam('entry')));
            $rating->set_source_alias('rating', 'value');
        }

        // Define id annotations
        $idea->annotate_ids('scale', 'scale');
        ...
        $idea->annotate_files('mod_idea', 'intro', null); // This file area hasn't itemid
        $content->annotate_files('mod_idea', 'content', 'id'); // By content->id

        // Return the root element (idea), wrapped into standard activity structure
        return $this->prepare_activity_structure($idea);
    }
}
