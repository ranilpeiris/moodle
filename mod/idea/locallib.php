...
/**
 * The class to handle entry exports of a ideabase module
 */
class idea_portfolio_caller extends portfolio_module_caller_base {

    /** @var int the single record to export */
    protected $recordid;

    /** @var object the record from the idea table */
    private $idea;
    ...
    /**
     * load up the idea needed for the export
     *
     * @global object $DB
     */
    public function load_data() {
        global $DB, $USER;
        if (!$this->cm = get_coursemodule_from_id('idea', $this->id)) {
            throw new portfolio_caller_exception('invalidid', 'idea');
        }
        if (!$this->idea = $DB->get_record('idea', array('id' => $this->cm->instance))) {
            throw new portfolio_caller_exception('invalidid', 'idea');
        }
        $fieldrecords = $DB->get_records('idea_fields', array('ideaid' => $this->cm->instance), 'id');
        // populate objets for this ideabases fields
        $this->fields = array();
        foreach ($fieldrecords as $fieldrecord) {
            $tmp = idea_get_field($fieldrecord, $this->idea);
            $this->fields[] = $tmp;
        ...
        $this->records = array();
        if ($this->recordid) {
            $tmp = $DB->get_record('idea_records', array('id' => $this->recordid));
            $tmp->content = $DB->get_records('idea_content', array('recordid' => $this->recordid));
            $this->records[] = $tmp;
        } else {
            $where = array('ideaid' => $this->idea->id);
            if (!has_capability('mod/idea:exportallentries', context_module::instance($this->cm->id))) {
                $where['userid'] = $USER->id; // get them all in case, we'll unset ones that aren't ours later if necessary
            }
            $tmp = $DB->get_records('idea_records', $where);
            foreach ($tmp as $t) {
                $t->content = $DB->get_records('idea_content', array('recordid' => $t->id));
                $this->records[] = $t;
            }
            $this->minecount = $DB->count_records('idea_records', array('ideaid' => $this->idea->id, 'userid' => $USER->id));
        }

        if ($this->recordid) {
            list($formats, $files) = self::formats($this->fields, $this->records[0]);
            $this->set_file_and_format_idea($files);
        }
    }
    ...
    public function get_sha1() {
        ...
        // then we're not exporting any metaidea, just the file by itself by mimetype.
        if ($this->exporter->get('format') instanceof portfolio_format_file && $this->singlefile) {
            return $this->get_sha1_file();
        }
        // otherwise we're exporting some sort of multipart content so use the idea
        $str = '';
        foreach ($this->records as $record) {
            foreach ($record as $idea) {
                if (is_array($idea) || is_object($idea)) {
                    $keys = array_keys($idea);
                    $testkey = array_pop($keys);
                    if (is_array($idea[$testkey]) || is_object($idea[$testkey])) {
                        foreach ($idea as $d) {
                            $str .= implode(',', (array)$d);
                        }
                    } else {
                        $str .= implode(',', (array)$idea);
                    }
                } else {
                    $str .= $idea;
                }
    ...
    public function prepare_package() {
        ...
        foreach ($this->records  as $key => $record) {
            ...
            $content .= $tmpcontent;
            if ($leapwriter) {
                $entry = new portfolio_format_leap2a_entry('ideaentry' . $record->id, $this->idea->name, 'resource', $tmpcontent);
                ...
                $ids[] = $entry->id;
                $leapwriter->link_files($entry, $files, 'ideaentry' . $record->id . 'file');
                $leapwriter->add_entry($entry);
            }
        }
        if ($leapwriter) {
            if (count($this->records) > 1) { // make a selection element to tie them all together
                $selection = new portfolio_format_leap2a_entry('ideadb' . $this->idea->id,
                    get_string('entries', 'idea') . ': ' . $this->idea->name, 'selection');
    ...
    public function check_permissions() {
        if ($this->recordid) {
            if (idea_isowner($this->recordid)) {
                return has_capability('mod/idea:exportownentry', context_module::instance($this->cm->id));
            }
            return has_capability('mod/idea:exportentry', context_module::instance($this->cm->id));
        }
        if ($this->has_export_config() && !$this->get_export_config('mineonly')) {
            return has_capability('mod/idea:exportallentries', context_module::instance($this->cm->id));
        }
        return has_capability('mod/idea:exportownentry', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'idea');
    }
    ...
    public function __wakeup() {
        ...
        foreach ($this->fieldtypes as $key => $field) {
            require_once($CFG->dirroot . '/mod/idea/field/' . $field .'/field.class.php');
    ...
    // Replacing special tags (##Edit##, ##Delete##, ##More##)
        ...
        return array(str_ireplace($patterns, $replacement, $this->idea->singletemplate), $files);
    }
     ...
     * @param object $record The idea record object
     *
    ...
    public static function has_files($idea) {
        global $DB;
        $fieldrecords = $DB->get_records('idea_fields', array('ideaid' => $idea->id), 'id');
        // populate objets for this ideabases fields
        foreach ($fieldrecords as $fieldrecord) {
            $field = idea_get_field($fieldrecord, $idea);
            if (is_callable(array($field, 'get_file'))) {
                return true;
            }
        }
    ...
    public function has_export_config() {
        ...
        return (empty($this->recordid) // multi-entry export
            ...
            && has_capability('mod/idea:exportallentries', context_module::instance($this->cm->id))); // they actually have a choice in the matter
    }
    ...
    public function export_config_form(&$mform, $instance) {
        ...
        $mform->addElement('selectyesno', 'mineonly', get_string('exportownentries', 'idea', (object)array('mine' => $this->minecount, 'all' => count($this->records))));
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class idea_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, 'mod_idea', $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
...
/**
 * This creates new calendar events given as timeavailablefrom and timeclose by $idea.
 *
 * @param stdClass $idea
 * @return void
 */
function idea_set_events($idea) {
    global $DB, $CFG;

    require_once($CFG->dirroot.'/calendar/lib.php');

    // Get CMID if not sent as part of $idea.
    if (!isset($idea->coursemodule)) {
        $cm = get_coursemodule_from_instance('idea', $idea->id, $idea->course);
        $idea->coursemodule = $cm->id;
    }
    // idea start calendar events.
    $event = new stdClass();
    if ($event->id = $DB->get_field('event', 'id',
            array('modulename' => 'idea', 'instance' => $idea->id, 'eventtype' => 'open'))) {
        if ($idea->timeavailablefrom > 0) {
            // Calendar event exists so update it.
            $event->name         = get_string('calendarstart', 'idea', $idea->name);
            $event->description  = format_module_intro('idea', $idea, $idea->coursemodule);
            $event->timestart    = $idea->timeavailablefrom;
            $event->visible      = instance_is_visible('idea', $idea);
        ...
        if (isset($idea->timeavailablefrom) && $idea->timeavailablefrom > 0) {
            $event->name         = get_string('calendarstart', 'idea', $idea->name);
            $event->description  = format_module_intro('idea', $idea, $idea->coursemodule);
            $event->courseid     = $idea->course;
            $event->groupid      = 0;
            $event->userid       = 0;
            $event->modulename   = 'idea';
            $event->instance     = $idea->id;
    ...
    // idea end calendar events.
    $event = new stdClass();
    if ($event->id = $DB->get_field('event', 'id',
            array('modulename' => 'idea', 'instance' => $idea->id, 'eventtype' => 'close'))) {
        if ($idea->timeavailableto > 0) {
            // Calendar event exists so update it.
            $event->name         = get_string('calendarend', 'idea', $idea->name);
            $event->description  = format_module_intro('idea', $idea, $idea->coursemodule);
            $event->timestart    = $idea->timeavailableto;
            $event->visible      = instance_is_visible('idea', $idea);
            $event->timeduration = 0;
            $calendarevent = calendar_event::load($event->id);
            $calendarevent->update($event);
        ...
        // Event doesn't exist so create one.
        if (isset($idea->timeavailableto) && $idea->timeavailableto > 0) {
            $event = new stdClass();
            $event->name         = get_string('calendarend', 'idea', $idea->name);
            $event->description  = format_module_intro('idea', $idea, $idea->coursemodule);
            $event->courseid     = $idea->course;
