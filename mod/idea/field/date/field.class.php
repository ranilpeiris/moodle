...
class idea_field_date extends idea_field_base {
    ...
    function display_add_field($recordid = 0, $formidea = null) {
        ...
        if ($formidea) {
            ...
            $content = (int)$DB->get_field('idea_content', 'content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid));
        } else {
            $content = time();
        }

        $str = '<div title="'.s($this->field->description).'" class="mod-idea-input form-inline">';
    ...
    function display_search_field($value=0) {
        ...
        $str = '<div class="form-inline">' . $selectors . ' ' . $datecheck . ' ' . get_string('usedate', 'idea') . '</div>';
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($content = $DB->get_field('idea_content', 'content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
