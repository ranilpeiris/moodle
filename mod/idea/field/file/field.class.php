...
class idea_field_file extends idea_field_base {
    var $type = 'file';
    ...
    function display_add_field($recordid = 0, $formidea = null) {
        ...
        // editing an existing ideabase entry
        if ($formidea) {
    ...
    function update_content($recordid, $value, $name='') {
        ...
        $usercontext = context_user::instance($USER->id);
        $files = $fs->get_area_files($this->context->id, 'mod_idea', 'content', $content->id, 'itemid, filepath, filename', false);
        ...
        if (count($files) == 0) {
            ...
            if (count($files) > 1) {
                ...
                debugging('more then one file found in mod_idea instance {$this->idea->id} file field (field id: {$this->field->id}) area during update idea record {$recordid} (content id: {$content->id})', DEBUG_NORMAL);
            }
        }
