...
class idea_field_url extends idea_field_base {
    var $type = 'url';
    /**
    ...
    function display_add_field($recordid = 0, $formidea = null) {
        ...
        if ($autolinkable) {
            ...
            $str .= '<span class="mod-idea-input">' . get_string('url', 'idea') . ':</span>';
            if (!empty($image)) {
                $str .= $image;
            }
            $str .= '</td><td>';
            ...
            $str .= '<tr><td align="right"><span class="mod-idea-input">' . get_string('text', 'idea') . ':</span></td><td>';
            ...
            $str .= $label;
            $str .= '<input type="text" name="field_'.$this->field->id.'_0" id="'.$fieldid.'" value="'.s($url).'"';
            $str .= ' size="40" class="mod-idea-input form-control d-inline" />';
        ...
        $module = array('name'=>'idea_urlpicker', 'fullpath'=>'/mod/idea/idea.js', 'requires'=>array('core_filepicker'));
        $PAGE->requires->js_init_call('M.idea_urlpicker.init', array($options), true, $module);
        $str .= '</div>';
        return $str;
    }
