...
class idea_field_latlong extends idea_field_base {
    var $type = 'latlong';
    ...
    var $linkoutservices = array(
          ...
          "Google Earth" => "@wwwroot@/mod/idea/field/latlong/kml.php?d=@ideaid@&fieldid=@fieldid@&rid=@recordid@",
          "Geabios" => "http://www.geabios.com/html/services/maps/PublicMap.htm?lat=@lat@&lon=@long@&fov=0.3&title=Moodle%20idea%20item",
    ...
    function display_add_field($recordid = 0, $formidea = null) {
        ...
        if ($formidea) {
            ...
            $lat = $formidea->$fieldname;
            $fieldname = 'field_' . $this->field->id . '_1';
            $long = $formidea->$fieldname;
        } else if ($recordid) {
        ...
        $classes = 'mod-idea-input form-control-static';
        $str .= '<label for="field_'.$this->field->id.'_0" class="' . $classes . '">' . get_string('latitude', 'idea');
    ...
    function display_search_field($value = '') {
        ...
        $return = html_writer::label(get_string('latlong', 'idea'), 'menuf_'.$this->field->id, false, $classes);
    ...
    function update_content($recordid, $value, $name='') {
        ...
        // the float to allow for a consistent float format in the ideabase.
    ...
    function export_text_value($record) {
        // The content here is from the ideabase and does not require location formating.
    ...
    public function field_validation($values) {
        ...
        return get_string('latlongboth', 'idea');
    }
}
