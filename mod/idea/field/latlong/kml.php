...
// A lot of this initial stuff is copied from mod/idea/view.php
...
$d       = required_param('d', PARAM_INT);   // ideabase id
$fieldid = required_param('fieldid', PARAM_INT);   // field id
$rid     = optional_param('rid', 0, PARAM_INT);    //record id

$url = new moodle_url('/mod/idea/field/latlong/kml.php', array('d'=>$d, 'fieldid'=>$fieldid));
if ($rid !== 0) {
    $url->param('rid', $rid);
}
$PAGE->set_url($url);
...
if ($rid) {
    ...
    if (! $field = $DB->get_record('idea_fields', array('id'=>$fieldid))) {
        print_error('invalidfieldid', 'idea');
    }
    if (! $field->type == 'latlong') { // Make sure we're looking at a latlong idea type!
        print_error('invalidfieldtype', 'idea');
    }
    if (! $content = $DB->get_record('idea_content', array('fieldid'=>$fieldid, 'recordid'=>$rid))) {
        print_error('nofieldcontent', 'idea');
    }
} else {   // We must have $d
...
echo idea_latlong_kml_top();

if($rid) { // List one single item
    $pm = new stdClass();
    $pm->name = idea_latlong_kml_get_item_name($content, $field);
    $pm->description = "&lt;a href='$CFG->wwwroot/mod/idea/view.php?d=$d&amp;rid=$rid'&gt;Item #$rid&lt;/a&gt; in Moodle idea activity";
    $pm->long = $content->content1;
    $pm->lat = $content->content;
    echo idea_latlong_kml_placemark($pm);
} else {   // List all items in turn

    $contents = $DB->get_records('idea_content', array('fieldid'=>$fieldid));

    echo '<Document>';

    foreach($contents as $content) {
        $pm->name = idea_latlong_kml_get_item_name($content, $field);
        $pm->description = "&lt;a href='$CFG->wwwroot/mod/idea/view.php?d=$d&amp;rid=$content->recordid'&gt;Item #$content->recordid&lt;/a&gt; in Moodle idea activity";
        $pm->long = $content->content1;
        $pm->lat = $content->content;
        echo idea_latlong_kml_placemark($pm);
    }
...
echo idea_latlong_kml_bottom();




function idea_latlong_kml_top() {
...
function idea_latlong_kml_placemark($pm) {
    return '<Placemark>
...
function idea_latlong_kml_bottom() {
    return '</kml>';
}

function idea_latlong_kml_get_item_name($content, $field) {
    global $DB;
    ...
    if($field->param2 > 0) {
        $name = htmlspecialchars($DB->get_field('idea_content', 'content', array('fieldid'=>$field->param2, 'recordid'=>$content->recordid)));
    ...
    if($name=='') { // Done this way so that "item #" is the default that catches any problems
        $name = get_string('entry', 'idea') . " #$content->recordid";
    }
