...
class idea_field_picture extends idea_field_base {
    var $type = 'picture';
    var $previewwidth  = 50;
    ...
    function display_add_field($recordid = 0, $formidea = null) {
        ...
        if ($formidea) {
            ...
            $itemid = clean_param($formidea->$fieldname, PARAM_INT);
            $fieldname = 'field_' . $this->field->id . '_alttext';
            if (isset($formidea->$fieldname)) {
                $alttext = $formidea->$fieldname;
            }
        } else if ($recordid) {
            if ($content = $DB->get_record('idea_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
                file_prepare_draft_area($itemid, $this->context->id, 'mod_idea', 'content', $content->id);
                if (!empty($content->content)) {
                    if ($file = $fs->get_file($this->context->id, 'mod_idea', 'content', $content->id, '/', $content->content)) {
        ...
        $str .= '<noscript>';
        if ($file) {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php/', $this->context->id.'/mod_idea/content/'.$content->id.'/'.$file->get_filename());
    ...
    // TODO delete this function and instead subclass idea_field_file - see MDL-16493
    ...
    function display_search_field($value = '') {
        return '<label class="accesshide" for="f_' . $this->field->id . '">' . get_string('fieldname', 'idea') . '</label>' .
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($template == 'listtemplate') {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_idea/content/'.$content->id.'/'.'thumb_'.$content->content);
            // no need to add width/height, because the thumb is resized properly
            $str = '<a href="view.php?d='.$this->field->ideaid.'&amp;rid='.$recordid.'"><img src="'.$src.'" alt="'.s($alt).'" title="'.s($title).'" class="list_picture"/></a>';

        } else {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_idea/content/'.$content->id.'/'.$content->content);
    ...
    function update_field() {
        ...
        // Get the old field idea so that we can check whether the thumbnail dimensions have changed
        $oldfield = $DB->get_record('idea_fields', array('id'=>$this->field->id));
        ...
        if ($oldfield && ($oldfield->param4 != $this->field->param4 || $oldfield->param5 != $this->field->param5)) {
            ...
            if ($contents = $DB->get_records('idea_content', array('fieldid'=>$this->field->id))) {
                $fs = get_file_storage();
                if (count($contents) > 20) {
                    echo $OUTPUT->notification(get_string('resizingimages', 'idea'), 'notifysuccess');
                    echo "\n\n";
                ...
                foreach ($contents as $content) {
                    ...
                    if ($thumbfile = $fs->get_file($this->context->id, 'mod_idea', 'content', $content->id, '/', 'thumb_'.$content->content)) {
                        $thumbfile->delete();
                    }
    ...
    function update_content($recordid, $value, $name='') {
        ...
        // Quickly make one now!
            ...
            $id = $DB->insert_record('idea_content', $content);
            $content = $DB->get_record('idea_content', array('id'=>$id));
        }
        ...
        switch ($names[2]) {
            case 'file':
                $fs = get_file_storage();
                file_save_draft_area_files($value, $this->context->id, 'mod_idea', 'content', $content->id);
                $usercontext = context_user::instance($USER->id);
                ...
                if (count($files) == 0) {
                    ...
                    if (count($files) > 1) {
                        // This should not happen with a consistent ideabase. Inform admins/developers about the inconsistency.
                        debugging('more then one file found in mod_idea instance {$this->idea->id} picture field (field id: {$this->field->id}) area during update idea record {$recordid} (content id: {$content->id})', DEBUG_NORMAL);
                    }

                    if ($file->get_imageinfo() === false) {
                        $url = new moodle_url('/mod/idea/edit.php', array('d' => $this->field->ideaid));
                        redirect($url, get_string('invalidfiletype', 'error', $file->get_filename()));
                    }
                    $content->content = $file->get_filename();
                    $this->update_thumbnail($content, $file);
                }
                $DB->update_record('idea_content', $content);
